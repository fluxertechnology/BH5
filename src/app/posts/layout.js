...
import { getHomeData } from "@/store/actions/pages/homeMainAction";
...
function PostsMain({ children }) {
  ...
  useEffect(() => {
    if (!Object.keys(state.homeData).length) useGlobalDispatch(getHomeData());
  }, [isMobile]);
  ...
  const clickTabLabel = (key) => {
    ...
    useGlobalDispatch(
      pushRoutes(post.pages.postMain.pages["postMain" + upCass + key.slice(1)])
    );
  };
  const floatBtnClick = () => {
    let user = state.user;
...
const PostsMainElement = styled.div.withConfig({
  shouldForwardProp: (prop) => !["showRightArea"].includes(prop),
})`
  /*  */
